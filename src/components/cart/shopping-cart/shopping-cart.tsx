...
import { OrderProduct, CreateOrderDTO } from "../../../interfaces/orders/order.interface";
    ...
    try {
      const mockOrderData : CreateOrderDTO = {
        ...
        orderProducts: shoppingCartItems.map((item) => {
          return {product: item.id, shippedFrom: "9e8fdec8-ab0a-4355-bcb5-47b8f19cd5b7", quantity: item.quantity} as OrderProduct
        })
      }
    ...
    <>
      <div className='header'>
        ...
        <div className='buttons'>
          <Link to='/checkout'><button onClick={handleCheckout}>CHECKOUT</button></Link>
        </div>
      </div>  
